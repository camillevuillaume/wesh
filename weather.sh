...
print_weather_short() {
  ...
  echo "${code_array[0]} | 📅 Date: $TODAY"
  echo "${code_array[1]} | 🌡️ ${temperature}°C (H: ${max_temp}°C L: ${min_temp}°C)"
  echo "${code_array[2]} | 🌬️ ${wind_speed} m/s (H: ${max_wind} m/s)"
  echo "${code_array[3]} | ☀️ $(date -d $sunrise +%H:%M) 🌙 $(date -d $sunset +%H:%M)"
  echo "${code_array[4]} | 💧 ${humidity}%"
